...
async def create_tracker(request: TrackerCreateRequest, db: db_dependency):
    try:
        ...
        new_tracker = Tracker(
            ...
            location_p=request.location_p if request.location_p is not None else datetime.now(),
            location_q=request.location_q if request.location_q is not None else datetime.now(),
            location_r=request.location_r if request.location_r is not None else datetime.now(),
            location_s=request.location_s if request.location_s is not None else datetime.now(),
            location_t=request.location_t if request.location_t is not None else datetime.now(),
            location_u=request.location_u if request.location_u is not None else datetime.now(),
            location_v=request.location_v if request.location_v is not None else datetime.now(),
            location_w=request.location_w if request.location_w is not None else datetime.now()
        )
...
@app.post("/get-license-plate")
async def get_license_plate(data: ImagePathRequest):
    try:
        license_plate = detection_call(data.image_path)
